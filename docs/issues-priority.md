...
| # | Status | Issue | Notes |
|---|--------|-------|-------|
| [#939](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/939) | 🔍 Pending validation | PDF diary: excessive blank band below the footer | Follow-up iPhone dump after #960 showed `.pdf-page-fill: NO` but `pinnedFooter=no`, leaving 44–138mm below short page images. Body/footer pixels are now copied unscaled onto an A4-height canvas and the footer is pinned at the ~10mm bottom margin; no fill strut is restored. Awaiting on-device confirmation. |
...
| [#951](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/951) | 🔍 Pending validation | Projection tab bar: match Settings Export segment styling | Shared `PeriodToggleGroup` with Export period (muted track, selected `bg-card` pill + shadow, `h-12` type). Sheet uses `bg-background` so the track reads like Settings (Pond `--card` === `--muted`). Awaiting on-device confirmation. |
...
| [#956](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/956) | 🔍 Pending validation | PDF share to Telegram: send PDF file, not blob: URL text | `shareOrDownloadPdf`: `navigator.share({ files: [new File([blob], name, { type: 'application/pdf' })] })` after `canShare({ files })`; download fallback. Never `url: blob:`. Wired into Settings PDF export and Day “Save as PDF”. Awaiting on-device confirmation (Telegram attachment). |
| [#957](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/957) | 🔍 Pending validation | Dates: one shared format everywhere; no chart label overlap | User-visible dates go through `formatLocalizedDate` (date-fns `PP`, Settings locale — e.g. `13 сент. 2026 г.`). Dashboard weight/custom axes, range pager, History rows, and Complete-the-day Week/Month/Year use that helper. Dense axes skip ticks instead of inventing `MM/DD/YYYY` or `DD.MM.YY`. Awaiting on-device confirmation. |
| [#958](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/958) | 🔍 Pending validation | PDF diary: footer gone, water gap, notes detached (regression after #939 fill removal) | #939 pin collapsed flex min-height and cropped the html2canvas clone. Pages now pack as in-flow **block** (no `.pdf-page-fill` under the footer). Capture clips through `.pdf-footer`; that slice is pinned to the A4 content canvas. Water rows and Notes header+body stay one card. pdfDebug #952 kept. Awaiting on-device confirmation. |
| [#959](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/959) | 🔍 Pending validation | Projection chart: add middle X-axis date (Month/Year too sparse after #957) | Month/Year X labels are start + mid-range grid tick + end via `formatLocalizedDate` (`PP`). `selectNonOverlappingDateTicks` keeps that middle index so a left-to-right skip cannot drop it; extras still drop when they would overlap. Week stays start+end. Awaiting on-device confirmation. |
