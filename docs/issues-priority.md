...
| [#642](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/642) | 📋 Not started | Write release-notes.ts entries during implementation, not at issue-closing time (#637 follow-up) | Spotted live 2026-08-07 while validating a batch: #637's `paths-ignore` fix works correctly, but the standing convention of writing `src/data/releaseNotes.ts` as part of the closing commit means every closing commit still touches a `src/` file and triggers a deploy. Proposal: write the release note at implementation time instead, so a pure validation-closing commit (doc row move only) stays `docs/**`-only and skips the deploy. Needs a CLAUDE.md tweak too |
