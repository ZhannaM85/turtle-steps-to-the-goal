...
| [#677](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/677) | ⬜ Open | Regression from #674: deleted active goal / wrong post-delete UX (stale data; refresh → empty form) | **Reopened again (2026-08-10).** Persist `skipPromotingNextActive` (8508f96) was wrong: after delete still shows stale «Текущая цель»; after refresh empty create form (#674 regression). Do not treat “no promoted goal” alone as success. Next agent must confirm expected UX with user / #674 before coding. Screenshots in chat. |
