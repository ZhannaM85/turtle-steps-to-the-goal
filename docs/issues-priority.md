...
| [#47](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/47) | 🔶 Fix pushed, awaiting live confirmation | History: From/To date filter inputs overlap on mobile | Third attempt: side-by-side with an explicit fixed `w-36` per input (not relative/shrink-based sizing), per direct suggestion — box no longer depends on the native control agreeing to shrink |
