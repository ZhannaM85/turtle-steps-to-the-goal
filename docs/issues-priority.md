...
| # | Status | Issue | Notes |
|---|--------|-------|-------|
| [#653](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/653) | 🔍 Pending validation | Android splash screen: turtle icon looks elongated/stretched | Root cause: `styles.xml`'s `AppTheme.NoActionBarLaunch` set `android:background` directly to the raw `@capacitor/assets`-generated `@drawable/splash` bitmap, which defaults to `Gravity.FILL` (non-uniform stretch to exact window bounds) — since each density bucket's splash bitmap is generated for a fixed reference aspect ratio that rarely matches a real device's screen exactly, the circular badge stretched into an oval. Fixed with a new `drawable/splash_screen_bg.xml` wrapper (`android:gravity="center"`) so the bitmap draws at native size instead of stretching; survives future `capacitor-assets generate` reruns since it only touches `@drawable/splash`, not the wrapper. `assembleDebug` build verified clean. Awaiting on-device confirmation |
