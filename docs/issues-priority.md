...
| [#846](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/846) | 🔍 Pending validation | Upgrade Vitest 4.x → 5.x (deliberate major migration) | Bumped `vitest` to ^5 + `@testing-library/jest-dom` to ^7. Added `src/vitest-jest-dom.d.ts` so Matchers use Vitest 5’s two-param form (jest-dom’s one-param Assertion no longer merges). Raised `testTimeout` to 15s for Vitest 5’s heavier per-file isolation cost on Windows. |
