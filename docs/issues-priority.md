...
| [#883](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/883) | 🔍 Pending validation | Body composition: visceral fat is shifted vs the other four metrics | 2-col pairing + each cell `grid-rows-[2.75rem_auto]` so values share a baseline when some ⓘ are missing. |
...
| [#885](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/885) | 🔍 Pending validation | Day: date input vs arrows height mismatch (regression of #809 / #647) | `DateInput` chrome is a div, so Day always uses `h-[2.625rem]` (the old WebKit skip was for a native date input). `py-1` kept. |
