...
| [#829](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/829) | 📋 Not started | Day: Next Morning Weight card from the following day's weigh-in | Read-only card below Night Eating: next calendar day's weight + signed delta vs today. Hide if D+1 has no weight (no skip-ahead). Derived only — no extra stored field. `next_morning_weight` on single-day export only, not interval/range. Neutral copy, no causation. |
