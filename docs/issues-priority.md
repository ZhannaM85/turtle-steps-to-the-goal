...
| [#43](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/43) | ⬜ Open | Redesign the Settings screen — feels inconsistent with the rest of the app | Raw `<input type="radio">` + unused `Card` component are the likely root cause; open question: Mood as `Select` vs. restyled `ToggleGroup` (keeps swatch preview) |
