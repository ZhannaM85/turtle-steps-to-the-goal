...
| [#49](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/49) | 🔶 Fix pushed, awaiting live confirmation | Dashboard charts: tapping anywhere navigates instantly, chart unusable for just viewing | Custom `Tooltip` `content` with an in-tooltip link, container `onClick` dropped. Link initially didn't work: Recharts' tooltip wrapper is `pointer-events:none` by default — fixed via `wrapperStyle={{pointerEvents:'auto'}}` |
...
| [#47](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/47) | 🔶 Fix pushed, awaiting live confirmation | History: From/To date filter inputs overlap on mobile | First fix (`grid grid-cols-2`) passed Chromium screenshots but still overlapped on real Safari — native date-input min-width ignores CSS sizing. New fix stacks From/To full-width instead of competing for horizontal space |
