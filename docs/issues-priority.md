...
| [#620](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/620) | 📋 Not started | Custom metric note Cancel button leaves an empty saved-looking entry instead of clearing it | Reported live right after confirming #619 on-device — canceling a brand-new note falls to the read-mode branch (empty text + pencil) instead of "nothing logged," since that branch is the only non-editing state the row has |
| [#621](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/621) | 📋 Not started | Meal list sort order: a very-late-night meal sorts first instead of last | Reported live with a screenshot — a 01:00 meal sorts above 14:09/15:23 meals on the same day. Related to #601 (day-start consistency) but a distinct consumer (#597's meal-list sort, not analytics bucketing) |
