...
| [#32](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/32) | ⬜ Open | Russian app name: change to «Черепашка идёт к цели» | Two occurrences in `ru.ts` (`nav.appName`, `about.intro`). Direct copy fix, ready to implement |
| [#33](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/33) | ⬜ Open | Russian nav label "О приложении" wraps to two lines — shorten to «О проекте» | `nav.about` in `ru.ts` only; About page's own `<h1>` title unaffected |
| [#34](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/34) | ⬜ Open | Weekly summary: drop the plus sign on gain weeks, keep the minus sign on losses | Swap `formatSignedNumber` → `formatNumber` in `WeeklySummaryCards.tsx`; default Intl sign behavior already does exactly this |
