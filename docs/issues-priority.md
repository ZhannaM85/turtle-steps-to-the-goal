...
| [#997](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/997) | 🔍 Pending validation | Repeat yesterday's meal shows empty «» when the meal name is cleared | The button and confirm title quoted the live name field, so a cleared title rendered empty quotes. They now use the positional meal type (Завтрак / Breakfast, …) — the same name an unlabeled meal already displays as — and still never yesterday's free-text. A blank name with no position falls back to the first meal type; the quote wrappers themselves omit quotes when the name is empty. Awaiting on-device confirmation |
| [#998](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/998) | 🔍 Pending validation | Meal search: remove duplicate barcode icon; keep Сканировать штрихкод tile | Add food had two scan entry points: the «Сканировать штрихкод» tile and a barcode icon inside Поиск…. The in-field icon is gone. The field is text search plus clear only; the tile still opens the scanner. Awaiting on-device confirmation |
| [#999](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/999) | 🔍 Pending validation | Add food: sticky header so close ✕ doesn’t overlap scroll content | The meal title (name, time, suggestions) is a shrink-0 sibling above the #996 scroll frame, so the dialog close stays in that band and list content scrolls below it. Awaiting on-device confirmation |
