...
| [#855](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/855) | 🔍 Pending validation | Day entries: delete option + confirmation modal | Saved Day values can be deleted after «Delete this entry?» confirm (Cancel leaves unchanged). × on an unsaved draft still clears without confirm; × on a saved note/steps/custom-metric note means delete. Same path for morning note, night reason, What helped, custom-metric values. Weight/sleep/body measurements/composition already had confirm-delete. |
