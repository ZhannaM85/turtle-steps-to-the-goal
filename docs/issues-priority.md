...
| [#582](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/582) | 🔍 Pending validation | Goal + Day: fiber target, What to track toggle, day totals | Re-scoped: Goal Suggest/Recalculate fiber (#341 field) + Settings What to track «Fiber» (default on) gates meal fiber fields, Day totals fiberG, and Remaining fiber card. |
| [#583](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/583) | 🔍 Pending validation | Settings Dishes: clarify what macros mean (per 100g vs portion) and strengthen field borders | Mode-aware `/100g` labels + bordered nutrition panel. Rework: edit border now wraps dish name input + nutrition (was name outside). |
| [#587](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/587) | 🔍 Pending validation | Dashboard: scrolling down shows Something went wrong error | Two root causes: (1) #581 always forcing fresh padded Recharts `domain` tuples — omit when unzoomed; memoize; restore NightEating X −0.5..1.5. (2) Live repro: `label.trim is not a function` in `mealSlotKeyForLabel` when DeferredMount hits LateMealCorrelation — historical numeric meal labels (#579) + #580 `effectiveTimeEaten`. Fix: coerce labels to string before `.trim()`. |
| [#588](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/588) | 🔍 Pending validation | Import: let user set Breakfast/Lunch/Snack/Dinner default times (not hardcoded) | Follow-up to #580. Import dialog + Settings prefs (`mealSlotDefaultTimesStore`); confirm saves clocks and stamps MFP meals; Day/History display uses same prefs for untimed slot labels. |
