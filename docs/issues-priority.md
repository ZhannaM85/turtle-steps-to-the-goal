...
| [#664](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/664) | 🔲 Open | Compare-to-yesterday/30-day arrow indicators for daily input fields (weight, steps, sleep, body composition) | Live arrow+text while editing (vs. yesterday), color-coded per-field since "up" isn't uniformly good (steps/sleep/muscle/bone/water: green=up; visceral fat/fat%/weight: green=down); disappears on save, replaced by an ⓘ icon next to the label with a tooltip (vs. yesterday + vs. exactly 30 days ago). Needs a Settings feature toggle. Open questions from filing resolved via follow-up comment: sleep follows the same up=green convention as steps; when yesterday has no entry, compare against the most recent prior entry and name that date (e.g. "compared to July 25th") instead of saying "yesterday" |
