...
| [#103](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/103) | ✅ Done | Home-screen icon looks broken — generic gray tile with a truncated name | New turtle-mark icon (oval shell + head + 4 legs, pond-mood teal/cream tokens), generated as PNGs (180/192/512) via a hand-rolled zero-dependency PNG encoder (Node's built-in `zlib` + a small CRC32/chunk writer — no image library added). `apple-touch-icon`/`apple-mobile-web-app-title`/`apple-mobile-web-app-capable`/`manifest.json`/`theme-color` wired into `index.html`. `favicon.svg` updated to the same mark (was a bare placeholder rectangle) |
