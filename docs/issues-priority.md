...
| [#306](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/306) | 🔍 Pending validation | iOS: camera permission for barcode scanning inside the native shell | `ios/App/App/Info.plist` gained `NSCameraUsageDescription` ("Camera access is used to scan food barcodes."). Same approach as Android #307 — no Capacitor Camera plugin; `@zxing/browser`'s `decodeFromVideoDevice` uses the web camera API, and Capacitor's WKWebView should surface the native prompt once the usage string exists. Needs on-device confirm (free Apple ID + Xcode on Mac is enough — see `docs/native-app-device-testing.md`): open barcode scanner → permission prompt → stream works; #291 manual entry still works if denied. |
