...
| # | Status | Issue | Notes |
|---|--------|-------|-------|
| [#666](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/666) | 📋 Not started | Goal page: past-goal weight values rounded to 1 decimal, losing precision | Reported live with a screenshot — "Прошлые цели" table's Status column (e.g. "58,9 → 58,7 кг") rounds to 1 decimal, losing precision from the actual logged entry. Root cause not yet investigated. |
| [#667](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/667) | 📋 Not started | Weekly goal reached on window's last day: celebration + new-goal unlock wrongly deferred to next day | Reported live: goal reached Sun 9 Aug (58.85→58.65 kg, -0.2 kg target) showed no celebration and stayed blocked from starting a new goal that day; the celebration modal instead popped a day late, on Mon 10 Aug, after a regain to 58.9 kg — "sounds like an evil joke" per the user. Mid-week early completion already celebrates immediately and correctly (confirmed out of scope); bug is specific to reaching the goal exactly on the window's last day. Spec (confirmed with user via clarifying questions): same-day immediate celebration + immediate new-goal unlock on the last day, no "hold until end of day" gating, and once reached that day it locks in (a later heavier re-weigh same day must not un-reach it). Root cause not yet investigated. |
| [#668](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/668) | 📋 Not started | Goal setup: no way to delete the current goal, only edit it | Reported live. Root cause / UI location not yet investigated. |
| [#669](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/669) | 📋 Not started | Day page: saving an empty weight value displays "не число" (NaN) instead of being blocked | Reported live with a screenshot — "Вес (кг)" field under "Утренние записи" shows NaN after saving empty. Should block the empty save instead. Root cause not yet investigated. |
| [#670](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/670) | 📋 Not started | Day page: allow deleting a weight entry (today or any date), with confirmation modal | Requested live, same report as #669. Root cause / UI location not yet investigated. |
