...
| [#960](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/960) | 🔍 Pending validation | PDF: stop stretching short pages to full A4; Заметки white-gap still broken | #958 pin drew every page onto a 1021×1487 (~276.7mm) canvas, so a 190px capture still filled A4 and inflated Water/Notes. Pages now pack top-down (no flex stretch) and place at **natural capture height**; leftover paper is letterbox below the image. pdfDebug reports stretch ratio and flags ≠ ~1. No `.pdf-page-fill`. Awaiting on-device confirmation. |
