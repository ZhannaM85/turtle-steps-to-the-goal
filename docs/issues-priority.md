...
| [#102](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/102) | 🟡 Partial | Blank screen when opening the app from the iOS home-screen shortcut | Shipped: `errorElement: <RouteErrorFallback />` on the top-level route (any render crash now shows a friendly reload prompt instead of unmounting silently — no error boundary existed anywhere before), plus a pure-CSS `#root:empty::before` spinner in `index.html` so the ~60s window is never truly blank. **Not resolved**: the actual root cause of why home-screen-shortcut mode specifically takes ~60s to first paint — couldn't be root-caused without direct device profiling access. "Check for updates" mechanism split out to #115 |
