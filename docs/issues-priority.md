...
| [#677](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/677) | 🔍 Pending validation | Regression from #674: deleted active goal comes back after refresh | Reopened incomplete (2026-08-10, on-device). Prior mid-session fix wasn't enough — full reload still promoted the newest remaining history goal via `getActiveGoal()`. Fixed by persisting `skipPromotingNextActive` on `useGoalStore` (zustand `persist` + localStorage, partialized to that flag only) so delete stays gone across refresh until the user saves a new goal; cleared again on `saveGoal` and on backup import. Awaiting on-device confirmation |
