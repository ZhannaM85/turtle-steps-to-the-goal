...
| # | Status | Issue | Notes |
|---|--------|-------|-------|
| [#79](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/79) | 🔲 Open | Manual meal "Add" button placement is confusing | Sits above the note input, so users don't realize it applies to the note they just typed. Plan: move both Add (full width, primary) and + Food down together, below the note/mood row, rather than leaving + Food stranded in the top fields row |
| [#80](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/80) | 🔲 Open | Meal note placeholder is confusing with the daily note | Both say "Заметка"/"Note" — indistinguishable at a glance. Plan: reword the meal-note placeholder to reference the dish itself (e.g. ru "Добавить своё блюдо?", en wording TBD) |
| [#81](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/81) | 🔲 Open | Group multiple food items under one meal | Bigger data-model change: `CalorieEntry` (meal group) gains `items: FoodItem[]`; note/mood/time-eaten move to the group level, items are name+kcal+macros only. Flexible grouping (no fixed Breakfast/Lunch/Dinner slots) — add an item into an existing meal or start a new one. Needs IndexedDB + export-bundle migration (existing entries become 1-item groups). See issue body for full design |
...
| [#83](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/83) | 🔲 Open | Food list is missing prepared/composite dishes (jam, stew, sandwich, pizza, pasta) | #78's sources were whole ingredients only. Candidate sources found: RU `health-diet.ru/base_of_meals/` (confirmed real per-100g table data, 13 dish categories); EN USDA FNDDS full CSV/JSON download (authoritative "as consumed" prepared-dish dataset, ~3.7MB zipped JSON). Parsing/curation deferred to implementation — see issue body |
| [#84](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/84) | 🔲 Open | Selected emotion icon not visible enough in dark mode | `EmotionPicker`'s selected state is `bg-muted`, too close to `--background` in dark mode; emoji ignore `text-foreground` so the background highlight is the only indicator. Shared component also drives the day-mood picker. Same class of bug as #11's WCAG contrast fixes |
| [#85](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/85) | 🔲 Open | Add a week-start preference (Monday vs the day I started tracking) | Weeks hardcoded to Monday-start (ISO) reads as wrong for mid-week starters. New `weekStartStore` (Settings toggle, like Units/Theme/Locale — "Start week when I started" / "Start week from Monday"), default `'monday'`. Blast radius verified narrow: only `currentWeekInfo.ts` and `weeklySummaries.ts` hardcode `startOfISOWeek`/`endOfISOWeek` — everything else (banner, Dashboard cards, celebration modal) derives from those two |
| [#86](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/86) | 🔲 Open | Can't reliably find or reuse a previously-added meal item | Two symptoms, one root need: note-field suggestion likely broken by iOS Safari's `<datalist>` limitation (plan: custom dropdown), and "+ Food" only searches `foods.ts`, never the personal `mealItems` library (plan: unify search, extend `MealItem` with optional last-used macros so it's actually reusable, not just a name) |
| [#87](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/87) | 🔲 Open | Rename the daily note field to disambiguate from meal notes | Follow-up to #80 — same "Заметка"/"Note" wording as the meal note nearby. Plan: heading → ru "Заметка дня" / en "Day's note"; new placeholder ru "Как прошёл день?" / en "Want to share anything for the day?" |
