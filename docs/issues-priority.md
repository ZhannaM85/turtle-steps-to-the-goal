...
| [#681](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/681) | 🔍 Pending validation | Past Targets shows Aug 4–9 goal as not reached though weight on Aug 8 met the 0.2 kg target | Root cause: Past Targets (and end-of-week nudge / weekly review / complete celebration) used `finalTargetMet` (#639) — latest weigh-in in the window — so Aug 8’s 58.4 (met) was overwritten by a later 58.65 as «not reached». Switched those surfaces back to sticky `targetMet`/`metOnDate`; Past Targets «to» weight uses new `metOnWeightKg` when reached. Pace check still uses `finalTargetMet`. Awaiting on-device confirmation |
