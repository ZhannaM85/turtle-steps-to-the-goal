...
| [#851](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/851) | 🔍 Pending validation | NoteEditRow: vertically center check + × (fixed size, no stretch) | Follow-up to #850. Row uses `items-center` so the fixed `icon-xl` check/× sit in the vertical middle of a multi-line field instead of at the top. Size/no-stretch from #850 unchanged. Applies to Day note, Morning note, Night Yes reason, Night No What helped. |
