...
| [#829](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/829) | 🔍 Pending validation | Day: Next Morning Weight card from the following day's weigh-in | Read-only card below Night food: next calendar day's weight + signed delta vs today. Hide if D+1 has no weight (no skip-ahead). Derived only — no extra stored field. `next_morning_weight` on one-day Send CSV only, not interval/range. Neutral copy, no causation. |
| [#830](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/830) | 🔍 Pending validation | Settings: All export leaves date inputs empty and filename is today only | Follow-up to #827. All selected: start/end dates blank; suggested name `turtle-steps-daily-log-2026-09-08`. Fill the real span and put that range in the filename. Custom unchanged. |
