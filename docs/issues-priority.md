...
| [#958](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/958) | 🔍 Pending validation | PDF diary: footer gone, water gap, notes detached (regression after #939 fill removal) | Follow-up: body and footer are captured separately, removing WebKit footer geometry from compositing; debug output records both canvas sizes and the inserted gap. Awaiting on-device export confirmation. |
| [#960](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/960) | 🔍 Pending validation | PDF: stop stretching short pages to full A4; Заметки white-gap still broken | New pdfDebug dump confirmed each page was composited into a fixed full-A4 slot with a growing blank band pinned above the footer (gap 117→446px, stretch up to 1.43). Fix: capture the whole styled page in one pass and place it at natural aspect — no footer pinning, no gap; short pages just leave normal paper below. Awaiting on-device export confirmation. |
...
| [#962](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/962) | ⬜ Open | PDF diary: move mood out of Показатели header onto the late-evening row | Follow-up to validated #941. Mood in the title wraps the header onto a second line; keep Sleep / Deep sleep / Steps in the header and put mood on the same body row as Ел(а) поздно вечером. Screenshot supplied. |
