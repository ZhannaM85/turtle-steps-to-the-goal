...
| [#643](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/643) | 🔍 Pending validation | Add meal: barcode not shown when the matched food already exists in DB | Root cause: picking a food from search/library results goes through `AddMealDialog`'s confirm-quantity step (`activeItem`), a separate JSX block from the manual-entry `MealItemEditorSheet` that #519 already fixed — the confirm-quantity screen never displayed a barcode at all. New `activeItemBarcode` (only possible for a `source: 'mealItem'` pick — the curated `FoodItem` catalog has no barcode field) rendered the same way #519 does. Awaiting on-device confirmation |
