...
| [#958](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/958) | 🔍 Pending validation | PDF diary: footer gone, water gap, notes detached (regression after #939 fill removal) | Footer compositing now uses its measured rectangle instead of an inferred bottom slice, so Water/Notes pixels remain in the page body. Confirmed in Макет PDF; awaiting on-device export confirmation. |
