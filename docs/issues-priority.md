...
| [#102](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/102) | 🔲 Open | Blank screen when opening the app from the iOS home-screen shortcut | Only via the home-screen shortcut, not a regular Safari tab; happens on cold open. Update: not a permanent hang — it eventually loaded after ~1 minute blank, with no loading indicator shown during that window. Not yet root-caused — no PWA manifest/`apple-mobile-web-app-capable` tag exists, no React error boundary anywhere, and it's unclear why home-screen mode specifically takes ~60s to first paint |
| [#103](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/103) | 🔲 Open | Home-screen icon looks broken — generic gray tile with a truncated name | `index.html` has no `apple-touch-icon`/`apple-mobile-web-app-title`, and `public/favicon.svg` is just a bare placeholder rectangle with no real logo yet. Needs an actual icon graphic + proper meta tags; likely worth a shared `manifest.json` alongside #102's missing-manifest gap rather than solving each separately |
