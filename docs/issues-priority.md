...
| [#65](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/65) | 🔲 Not started | Add a time-eaten field to meals, for intermittent fasting tracking | Time-of-day only (`HH:MM`), defaults to current time, optional/clearable. Reordering meals does NOT clear time (decided against the original "reset on reorder" proposal — data-loss risk). No fasting-window calculation, capture only |
...
| [#62](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/62) | 🔲 Not started | Add a local food/nutrient database with quantity-based entry | **Blocked on #51** — data model needs protein/fat/carbs, not just calories. Large: dataset sourcing (en+ru) is the real cost, plus a new quantity-based entry flow alongside manual entry |
