...
| [#35](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/35) | ⬜ Open | About page: author credit doesn't link to GitHub | "Автор: zhannam85" is plain text with no `<a>`; should link to the GitHub profile |
| [#36](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/36) | ⬜ Open | Allow reordering meal entries via drag and drop | `calorieEntries[]` is add-order-only today; likely needs a DnD library (e.g. `@dnd-kit/core`) |
| [#37](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/37) | ⬜ Open | Move the kg/lb unit toggle from the Goal page to Settings | `displayUnit` is a `Goal` field today, read in 27 files; decide during implementation whether it stays a `Goal` field or becomes an app-level preference |
| [#38](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/38) | ⬜ Open | Remind the user to set next week's goal at the end of the week | No push infra exists; likely an in-app quiet banner keyed off `useCurrentWeekInfo` (#18), matching the app's no-pressure tone |
| [#39](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/39) | ⬜ Open | History: no read-only way to see a day's full details (meals, notes, emotions) | `EntryRow` only has `'view'` (collapsed) and `'edit'` (`alwaysEditable`) modes; needs a read-only expanded state |
