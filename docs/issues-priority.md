...
| [#883](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/883) | 🔍 Pending validation | Body composition: visceral fat is shifted vs the other four metrics | Live grid stays **3-across** (#515 compact). Values share a `min-h-5` label row; comparison ⓘ is `size-4` in this grid so it cannot blow the card. |
| [#884](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/884) | 🔍 Pending validation | Day date Russian label is jammed without spaces | Dropped `truncate`. Day field is `w-48 shrink-0`. Native overlay text is forced transparent so it cannot overprint the `PP` label. |
| [#885](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/885) | 🔍 Pending validation | Day: date input vs arrows height mismatch (regression of #809 / #647) | Default chrome is `h-12` (#725). Day overrides to `h-[2.625rem]` with no `h-8`/`py-1`. Overlay is `h-full min-h-0`. |
| [#886](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/886) | 🔍 Pending validation | Info tooltip sits too far from the info icon | Popover anchors to the ⓘ glyph, not the 44px #872 hit box. `align="center"` + `sideOffset={4}`. |
