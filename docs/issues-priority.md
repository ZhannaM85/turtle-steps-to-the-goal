...
| [#663](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/663) | 🔲 Open | Encouraging micro-notifications for nutrient-rich meals (protein, fiber, vitamins) | Requested from the meal-composition screen. Brainstormed a broader list in the issue (hydration, macro balance, variety, streaks). Open question: whether fiber/vitamins are tracked in the food data model at all today, or only protein/fat/carbs as shown on screen — may need to scope first pass to already-tracked macros. Should likely reuse the Day screen's existing card + eye-icon show/hide pattern rather than new notification UI |
