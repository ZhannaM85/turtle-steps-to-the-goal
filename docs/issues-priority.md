...
| [#648](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/648) | 🔍 Pending validation | Custom `ThemeBridgePlugin` (#308) duplicates Capacitor 8's built-in `SystemBars` plugin | Removed `ThemeBridgePlugin.java` and `@capacitor/status-bar` entirely — `src/shared/native/nativeChrome.ts` now calls `@capacitor/core`'s built-in `SystemBars.setStyle()` (no separate install, cross-platform, one call themes both bars). Left `insetsHandling` at its default rather than `'disable'`: that would also remove the plugin's real inset-passthrough/keyboard-padding logic, not just the CSS-fallback injection that raced the console error — not worth the regression risk to #308's confirmed-working safe area behavior. Confirmed on-device via `adb`: no crashes, theming still correct, console error did not reproduce on this run (inherently a race, not guaranteed gone) |
| [#650](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/650) | 🔍 Pending validation | History: "Edit this day" button doesn't navigate to that day | Root cause: by design since #48, the button only switched History's own List view to a filtered single day rather than leaving the page. Now navigates to the Day screen for that date via `?date=` — the same deep-link `TodayScreen` already reads (#200), not a new mechanism. `HistoryScreen.tsx`'s `editDayFromCalendar`. Reported by the user as still showing old behavior on the iOS PWA — reopened #649 rather than assuming it's fixed; investigating whether they're connected (stale cached PWA bundle) |
| [#649](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/649) | 🔲 Reopened | PWA: "update available" banner keeps reappearing even after reloading | Was closed as "explained by #211 + deploy frequency" from log analysis alone, without on-device confirmation — reopened after the user pointed out closing without device validation isn't allowed. Newly relevant: while checking #650, the user reported its fix not visibly taking effect on the iOS PWA, symptoms consistent with the same stale-cache mechanism. Investigating both together |
