...
_Shared Safari-safe clocks. #858 already archived as validated the same day. #872–#879 are a one-off design/UX audit (analysis only; do not implement until picked)._

| # | Status | Issue | Notes |
|---|--------|-------|-------|
| [#879](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/879) | 📋 Not started | Empty and error states: meals, libraries, RouteErrorFallback | `EmptyState` only covers Day-no-goal / History / Dashboard. Meals, water, recipe/metric libraries, and `RouteErrorFallback` are plain text or a different stack. |
| [#878](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/878) | 📋 Not started | Tokenize semantic status colors | Reason dots, entry-comparison emerald/orange, and Goal overlap (`GoalForm.tsx`) still use raw Tailwind after #734. Moods never tint them. |
| [#877](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/877) | 📋 Not started | Settings: group ~29 peer cards instead of one flat scroll | `SETTINGS_CARD_KEYS` is one `gap-4` list. What-to-track is a page-inside-a-page. Collapse-all is a 14px text link. |
| [#876](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/876) | 📋 Not started | Day accordion recipe: headers, stats nesting, redundant page blurb | Morning/Evening have icon+subtitle; Water/totals are muted-only; Stats splits the trigger and nests StatCards in a second `section-shell`. |
| [#875](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/875) | 📋 Not started | Unify confirm, undo, and discard bars | `ConfirmDeleteEntryBar` exists; meal confirm, Add-meal discard, undo toast, and backup reminder still hand-roll cousins. |
| [#874](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/874) | 📋 Not started | One Chip primitive for water, meal names, and History legend | Water = pill + ×; meal templates (#845) = `Button sm`; History legend = bordered pill. Same job, three languages. |
| [#873](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/873) | 📋 Not started | Settings cards still use number-card chrome; titles collide with pin | #735 leftover: `SettingsPinnableCard` wraps `Card`. Pin/collapse are absolute with no header padding (worse in RU). |
| [#872](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/872) | 📋 Not started | Title-row icon actions stay under 44pt after #858 | `DayFieldHeader` / Settings pin / meal pencil are `icon-sm` (28px); water × and StatCard eye are `icon-xs`; `InfoTooltip` is 16px. |
