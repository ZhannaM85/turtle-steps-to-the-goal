...
| [#648](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/648) | 🔲 Open | Custom `ThemeBridgePlugin` (#308) duplicates Capacitor 8's built-in `SystemBars` plugin | Found reading `adb logcat` during #310 work — Capacitor 8's Android core already ships a native `SystemBars` plugin doing overlapping work (window insets, safe-area CSS, status/nav bar style). App works correctly on-device despite the overlap (custom plugin fires later, wins), but there's a harmless-looking cold-boot console error (`Error injecting safe area CSS: ... reading 'style' of null`) from Capacitor's own code worth understanding, and a cleanup opportunity to lean on the built-in plugin instead of the custom one. Low priority |
| [#649](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/649) | 🔲 Open | PWA: "update available" banner keeps reappearing even after reloading | Reported live with a screenshot (iOS PWA, Settings shows stale `v600`). Flagged by the user as a potential regression from #310's service-worker registration change. Not yet confirmed either way — could also be explained by today's 5 back-to-back real deploys plus #211's already-documented GitHub Pages CDN sw.js caching. Needs actual investigation, not guessing |
| [#650](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/650) | 🔍 Pending validation | History: "Edit this day" button doesn't navigate to that day | Root cause: by design since #48, the button only switched History's own List view to a filtered single day rather than leaving the page. Now navigates to the Day screen for that date via `?date=` — the same deep-link `TodayScreen` already reads (#200), not a new mechanism. `HistoryScreen.tsx`'s `editDayFromCalendar` |
