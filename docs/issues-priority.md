...
_Day КБЖУ / export: goal vs actual, omit untracked fields, date-label next-morning weight, morning/evening notes, and night-food overnight span._
...
| # | Status | Issue | Notes |
|---|--------|-------|-------|
| [#893](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/893) | 🔍 Pending validation | Day date nav: drop Date label; put entry checkmark on the date row | Visible «Дата»/Date label removed; `aria-label` kept. Has-entry check sits after next, before the Today slot. |
| [#890](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/890) | 🔍 Pending validation | Confirm before deleting meal-name templates (and other trash actions) | `ConfirmDeleteEntryBar` on meal-name templates, recipes, custom metric/correlation defs, custom eating reasons. Saved foods already confirmed. |
| [#892](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/892) | 🔍 Pending validation | PDF footer date/disclaimer overlaps page content | Footer uses each page’s height; tables reserve a bottom band so disclaimer/«Создано» never sit on rows. |
