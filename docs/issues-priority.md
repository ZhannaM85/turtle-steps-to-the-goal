...
| [#615](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/615) | 🔍 Pending validation | Clearer period context when cycle tracking is on | New one-line factual note on `WeightTrendChart.tsx` (no prediction logic). History's calendar legend already correctly gated per-type, no change needed. Refined live: originally showed on every paged range whenever cycle tracking was on; now also requires the currently-viewed range to actually have a logged period day. Awaiting on-device confirmation |
...
| # | Status | Issue | Notes |
|---|--------|-------|-------|
| [#631](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/631) | 🔍 Pending validation | Unusual-data-point links can land on a day with no logged data | Only affected caller was `CorrelationView.tsx` (#522) — every other `OutlierPointsList` user keys/navigates by day already. `getDate` now resolves to the earliest date within the flagged week that actually has a logged weight (guaranteed to exist, since `weeklySummaries` only sets `deltaVsPriorWeekKg` when the week's own `averageWeightKg` is non-null), instead of always the week's Monday |
...
| [#633](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/633) | 🔍 Pending validation | PDF section picker: enabled state doesn't match Settings' What to track | Root cause: `pdfSectionAvailability()` was a pure "has any data" check with zero awareness of any Settings tracking toggle. New `gatePdfSectionAvailability()` ANDs it against `useTrackedFieldsStore` (sleep/steps/bodyMeasurements/bodyComposition/nightEating) plus the older per-field opt-in stores (cycle/digestion/alcohol/water) at the `ExportSection.tsx` call site; `weightTrend`/`weeklyAverages` have no toggle so pass through unchanged |
