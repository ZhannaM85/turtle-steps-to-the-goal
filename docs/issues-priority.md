...
| [#594](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/594) | 🔍 Pending validation | JSON backup: export and import all Settings page preferences | Bundle v10 `settings` blob: unit, week-start, day-start, meal-slot defaults, What-to-track / cycle / digestion / water / electrolytes, reminder, trend-series safety, profile, meal-label presets. Older backups leave device prefs alone. |
| [#595](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/595) | 🔍 Pending validation | Settings: confirm applying new default meal times to existing untimed meals | On blur after a slot-default change: if any untimed Breakfast/Lunch/Snack/Dinner meals exist, alertdialog Yes (stamp via `stampSlotDefaultsOnUntimedMeals`) / No (prefs only). |
