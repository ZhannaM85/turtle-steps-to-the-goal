...
| [#44](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/44) | ⬜ Open | Add an overall mood-of-the-day, next to the daily note | New `DailyEntry.emotion?`, reuses existing `Emotion` type + `EmotionPicker`; likely bundled into the existing `saveNote()` action |
...
| [#49](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/49) | ⬜ Open | Dashboard charts: tapping anywhere navigates instantly, chart unusable for just viewing | Reverses #41's tap-anywhere approach; needs a custom Recharts `Tooltip` `content` component with an in-tooltip link, dropping the container-level `onClick` |
...
| [#47](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/47) | ✅ Done | History: From/To date filter inputs overlap on mobile | Switched `flex flex-wrap` + `max-w-40` to `grid grid-cols-2` + `min-w-0`, a deterministic equal-width track instead of ambiguous auto-sizing; verified at 360px/390px |
| [#48](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/48) | ⬜ Open | History: add a calendar view alongside the current list view | Month grid, marked days, tap-to-see-detail below; reuse #39's read-only detail rendering + #43's `ToggleGroup` for the List/Calendar switch |
