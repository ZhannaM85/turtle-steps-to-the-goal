...
| [#677](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/677) | 🔍 Pending validation | Regression from #674: delete active goal should pop stack to previous goal | **Stack UX (user 2026-08-10):** delete pops top; previous goal shows immediately + after refresh. Undid persist `skipPromotingNextActive` (was opposite). `deleteGoal` now removes active then `getActiveGoal()` → previous; GoalForm prefers `existingGoal` over deleted snapshot (snapshot only when stack empty, #674). Awaiting on-device confirmation |
| [#681](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/681) | 🔍 Pending validation | Past Targets shows Aug 4–9 goal as not reached though weight on Aug 8 met the 0.2 kg target | **Corrected RCA:** bad baseline («от 58,75» vs Day 4 Aug **58,85**), not sticky reach. `resolveBaselineWeightKg` now prefers weekStart day’s weigh-in over frozen prior-day snapshot; keep `finalTargetMet` for Past Targets. Fresh saves also snapshot weekStart weight when logged. Awaiting on-device confirmation |
| [#683](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/683) | 📋 Not started | Do not restrict goal start date; warn on overlap with previous goal | Allow any start date. If it overlaps a previous goal window, warn but do not block pick/save. |
| [#684](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/684) | 📋 Not started | Food library: sort by title (A–Z / Z–A) and by date added | Sort controls near count/search: title asc/desc + date-added latest↔earliest. |
