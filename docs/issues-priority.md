...
| [#29](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/29) | ⬜ Open | Weekly summary: de-emphasize gain weeks instead of full bold treatment | Agreed direction: keep both directions visible, but losses stay bold/celebratory while gains render smaller/quieter — nothing hidden. Open questions: exact quiet styling, zero-change weeks |
| [#31](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/31) | ⬜ Open | Remove the single Update/Log entry button — Weight, Note, and meals each save immediately | Supersedes #30. No shared submit button; Weight/Note get their own inline Save (Check icon, matching meal edits); meal Add/Save/Delete already persist per-action. Ready to implement |
