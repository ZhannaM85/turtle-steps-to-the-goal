...
Active work queue only (open, pending validation, not started, partial). Closed history lives in [`docs/issues_priority_archived.md`](./issues_priority_archived.md) (archived 2026-08-04 because the combined file was too long for Preview — only through ~Tier 51 was visible).
...
| [#586](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/586) | 🔍 Pending validation | Goal: weekly pace summary rounds 0.28 kg to 0.3 | Used `formatExactNumber` (max 2 decimals) on Goal/Day StatCards, GoalForm summary, and Past targets — was `formatNumber` default 1 decimal. |
