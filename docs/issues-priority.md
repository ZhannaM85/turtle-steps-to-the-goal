...
| [#28](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/28) | ⬜ Open | Add per-meal notes and a happy/unhappy/neutral emotion tag to calorie entries | Decisions settled (see issue comments) — `CalorieEntry` gains `note?`/`emotion?`; both entered directly in the Add flow, not gated behind pencil-to-edit; ready to implement |
