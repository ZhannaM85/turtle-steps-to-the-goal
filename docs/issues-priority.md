...
| # | Status | Issue | Notes |
|---|--------|-------|-------|
| [#917](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/917) | 🔍 Pending validation | PDF diary: compact day layouts and show complete body composition | Uses one packable diary stream: a date stays with its first card, while remaining cards and following days share the next styled sheet; combines acne with its note and sleep with deep sleep; renders muscle, visceral fat, body water, bone mass, and body fat together. |
...
| # | Status | Issue | Notes |
|---|--------|-------|-------|
| [#918](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/918) | 🧪 Local preview | PDF diary export: blank page appears between Food and Water | Tightened the styled-page fit limit to stay inside the PDF printable area and avoid a nearly blank image slice; regression test covers Food followed by Water at the prior boundary. Awaiting local PDF review before push and on-device validation. |
| [#919](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/919) | 🧪 Local preview | PDF diary export: reduce or remove repeated page disclaimer | Chose the requested smaller-footer option: the disclaimer and creation date now sit near the bottom of each styled sheet at 6.5pt. Awaiting local PDF review before push. |
| [#920](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/920) | 🧪 Local preview | PDF diary export: day starts partway down a page | Each date now owns a starting PDF sheet, while a long day's cards can continue onto later sheets. Awaiting local PDF review before push. |
| [#921](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/921) | 🧪 Local preview | PDF summary export: arrange compact metric cards two per row | Summary metric cards use two columns; charts and weekly-averages tables remain full width. Continuation sheets retain the grid. Awaiting local PDF review before push. |
| [#922](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/922) | 🧪 Local preview | PDF diary export: improve local generation speed for weekly diaries | Reduced each standard PDF page canvas from 2x to 1.5x resolution, cutting render pixels by about 44% while retaining ~144dpi output. Awaiting local PDF review. |
