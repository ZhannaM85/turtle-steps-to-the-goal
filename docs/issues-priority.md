...
| [#643](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/643) | 🔲 Open | Add meal: barcode not shown when the matched food already exists in DB | Reported live with a screenshot — picking an existing DB food to log a meal populates name/brand/macros but never shows a barcode, even if the matched food has one on record. Log-only, not yet implemented |
