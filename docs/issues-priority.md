...
| [#677](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/677) | ⬜ Open | After deleting the new active goal, show the previous remaining goal (view mode), including after refresh | **Expected UX confirmed (2026-08-10):** e.g. history has Aug 4–9 @ 0.2 kg; user starts Aug 10–16 @ 0.3 kg then deletes it → UI must show Aug 4–9 @ 0.2 in view mode; refresh must show the same. Not deleted-goal snapshot, not empty create form, not skip-promote. Wrong: persist `skipPromotingNextActive` (8508f96) and `justDeletedGoal` of the deleted record. Right: delete active → promote newest remaining via `getActiveGoal` into store/view. |
| [#681](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/681) | ⬜ Open | Past Targets shows Aug 4–9 goal as not reached though weight on Aug 8 met the 0.2 kg target | **Reopened / start over (2026-08-10).** Wrong fix (sticky `targetMet` instead of `finalTargetMet`) reverted (`757e43b`). Real bug: starting («from») weight was **58.75** while Day for weekStart **4 Aug** shows **58.85** — bad baseline, not end-of-week formula. Keep `finalTargetMet` for reached/missed (sticky mid-week reach causes swinging weeks). Fix baseline / `resolveBaselineWeightKg` so weekStart day’s weigh-in wins. #682 was duplicate. |
