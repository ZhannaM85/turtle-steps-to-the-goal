...
| # | Status | Issue | Notes |
|---|--------|-------|-------|
| [#666](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/666) | 🔍 Pending validation | Goal page: past-goal weight values rounded to 1 decimal, losing precision | Root cause: `PastTargetsList.tsx`'s baseline/current weigh-in line used `formatNumber` (defaults to 1 decimal) instead of `formatExactNumber` (entered precision, up to 2 decimals) — the target-per-week figure on the same row already used `formatExactNumber` correctly. Fixed to match. |
| [#667](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/667) | 🔍 Pending validation | Weekly goal reached on window's last day: celebration + new-goal unlock wrongly deferred to next day | Root cause: `goalWindowHasEnded` (strict `today > weekEnd`) gated both the celebration's 'complete' phase and the restart button, so on weekEnd itself (today === weekEnd) neither fired even though the target was already met that day. Added `goalWindowConcluded` (also true when reached on weekEnd itself) and swapped it in at all three call sites (`useWeeklyGoalCelebration`, `GoalScreen`'s nudge phase, `GoalForm`'s restart gate via a new `activeGoalConcluded` prop). Also added a `reachedOnLastDayWeekStart` lock in `goalCelebrationStore` (set by `useActiveGoalProgress` the moment reached-on-weekEnd is first detected) so a later same-day re-weigh that overwrites today's single `DailyEntry.weightKg` with a heavier value can't un-reach it, per spec point 5. |
...
| [#669](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/669) | 🔍 Pending validation | Day page: saving an empty weight value displays "не число" (NaN) instead of being blocked | Root cause: `weightSchema` is `.optional()` (a day can legitimately go untracked), so `saveWeight()`'s `safeParse` treated an empty field as a *valid* clear — it persisted `weightKg: undefined` and flipped to read-only display mode, which then rendered `formatExactNumber(undefined)`; `Intl.NumberFormat` formats that as literal "NaN" (ru: "не число"). Fixed by rejecting `result.data === undefined` in `saveWeight()` specifically (`useDailyEntryFormState.ts`) — shows the existing "Invalid value" error and stays in edit mode instead of saving. Awaiting on-device confirmation |
