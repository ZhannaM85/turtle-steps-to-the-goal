...
| [#21](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/21) | ⬜ Open | Read-only display + pencil-to-edit for Weight/Note; itemized calorie entries | Supersedes #20's "Undo last add" with per-entry editing. Design decisions settled (see issue comments) — `caloriesConsumed` becomes `calorieEntries[]` with an IndexedDB + export-bundle migration; ready to implement |
| [#22](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/22) | ⬜ Open | Replace default favicon with a turtle-steps-themed icon | Current `favicon.svg` is a generic placeholder shape, unrelated to the app's branding |
| [#23](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/23) | ⬜ Open | Add an About page — what this project is and who made it | Blocked on author bio/project copy. Gets its own dedicated tab-bar entry (pairs with #24 freeing the slot) |
| [#24](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/24) | ⬜ Open | Fold Export into the Settings screen, remove standalone Export tab | Frees a tab-bar slot for #23's About entry; keeps total tabs at 6. Self-contained nav restructuring, no domain changes |
| [#25](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/25) | ⬜ Open | Make the top header sticky instead of scrolling with content | `AppShell.tsx` header has no `sticky`/`fixed` treatment; bottom tab bar already does. Small CSS/layout fix |
