...
| [#745](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/745) | 🔍 Pending validation | Add trash to delete sleep and body composition, with the same confirm as weight | Trash + inline Yes/No on Sleep, Body composition, and Body measurements. Pencil then Trash (#746). Clears that card’s saved fields. |
...
| [#749](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/749) | 📋 Not started | Separate Settings toggles for Zepp and AutoSleep screenshot fill | Per-source, not one master switch. Zepp OCR toggle only if body composition is tracked; AutoSleep OCR toggle only if sleep is tracked. Default on. Hides that source’s ImageUp without turning off the field. |
