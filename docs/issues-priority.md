...
| [#28](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/28) | ⬜ Open | Add per-meal notes (and optionally an emotion tag) to calorie entries | `CalorieEntry` gains `note?`/maybe `emotion?`. Open questions: daily-note interplay, whether emotion is in scope, entry mechanics (pencil-to-edit vs. quick-add), tone of any emotion set given the app's low-pressure ethos |
