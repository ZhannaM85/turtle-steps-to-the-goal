...
| # | Status | Issue | Notes |
|---|--------|-------|-------|
| [#698](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/698) | 🔍 Pending validation | Upgrade react-router-dom past the RSC CSRF advisory | Bumped `react-router-dom` / `react-router` **7.18.1 → 7.18.2** (first patched release for [GHSA-qwww-vcr4-c8h2](https://github.com/advisories/GHSA-qwww-vcr4-c8h2)). No `--force`. `tsc` + lint + `router.test.tsx` green. No release note (no user-facing change). Confirm Day / History / Settings / Goal still navigate. |
| [#699](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/699) | 🔍 Pending validation | Patch brace-expansion in the production lockfile | `npm audit fix` (no `--force`): brace-expansion 1.1.16→1.1.18 (exceljs/glob), 2.1.2→2.1.4 (readdir-glob + PWA filelist), 5.0.7→5.0.9 (eslint). Same lockfile pass also non-force-bumped other patched transitives (`undici` 7.29.0, `@hono/node-server`, `fast-uri`, `hono`, `postcss`/`nanoid`). `exportXlsx.test.ts` green. Leftover force-only items stay on #700/#701. Confirm Excel export still works. |
| [#700](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/700) | 🔍 Pending validation | Resolve exceljs uuid advisory without downgrading exceljs | exceljs still 4.4.0 (no upstream). Nested `overrides.exceljs.uuid` = **11.1.1**. exceljs only uses `uuid.v4()` (`cf-rule-ext-xform.js`), not the vulnerable v3/v5/v6+`buf` path. `exportXlsx.test.ts` green. Capacitor `xcode`'s uuid@7 stays for #701. Confirm Excel export still works. |
| [#701](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/701) | 🔍 Pending validation | Triage Capacitor CLI and other devDependency advisories | Production `npm audit --omit=dev` is **0**. Patched: `overrides.tar` = 7.5.22 (critical; CLI stays 8.5.0, `npx cap --version` ok). **Accepted** (no forward bump without downgrade): `elliptic` via `vite-plugin-node-polyfills@0.28.0`; `minimatch` via `@capacitor/assets`/`replace` (no fix); `sharp` libvips via `@capacitor/assets` (no fix); `xcode`'s `uuid@7` (uuid 11 would be ESM vs `require`). |
| [#702](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/702) | 🔍 Pending validation | Enable GitHub CodeQL for our TypeScript | `.github/workflows/codeql.yml` (`javascript-typescript`, `build-mode: none`, CodeQL Action v4). Push/PR to main + weekly. Does not fail CI on alerts. Default-setup API 404; advanced workflow instead. Confirm the first scan appears under Security → Code scanning. |
...
| [#709](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/709) | 📋 Not started | Settings: list all Dashboard graphs with show/hide toggles | Catalog every `DashboardChartKey` in Settings; same store as Dashboard eye toggles. Custom correlations out of scope. |
| [#708](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/708) | 📋 Not started | Dashboard graphs must not silently disappear when empty | Stop `return null` on empty correlation/chart cards — keep title + empty copy. Feature-gated omit (alcohol off) OK. |
