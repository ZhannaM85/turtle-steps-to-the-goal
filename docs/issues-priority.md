...
| [#45](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/45) | ⬜ Open | Dashboard charts: tapping a point doesn't navigate on mobile | Regression against #41; likely a Recharts touch/onClick quirk, not a `resolveChartClickDate` logic bug — needs real-device or touch-emulation testing |
| [#46](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/46) | ⬜ Open | Remove the weight projection/prognosis line from the dashboard chart | `goal` prop and `projectedPaceTrajectory` become fully unused afterward — delete outright, not just stop calling; simplifies #41's click-resolution too |
| [#47](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/47) | ⬜ Open | History: From/To date filter inputs overlap on mobile | Likely native `<input type="date">` minimum width exceeding the `max-w-40` cap on narrow viewports; needs real-device/mobile-viewport verification |
