...
| # | Status | Issue | Notes |
|---|--------|-------|-------|
| [#771](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/771) | 🔍 Pending validation | AutoSleep Today screenshot fill: deep sleep 7h 10m instead of 45 minutes (z icon) | On-device 2026-08-29: total sleep 9h 22m correct; confirm dialog deep sleep 7h 10m (star / quality) instead of 0h 45m (circled z). Follow-up: after #772, deep was empty — `0h 45m` sat on a `Sleep Rating` line skipped for the word “rating”. Word `rating` is no longer a skip; all `Xh Ym` in the OCR text are scanned for a plausible deep (< 4h). |
...
| [#774](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/774) | 📋 Not started | Allow multi-select for Why am I eating? | On-device 2026-08-29: dropdown is single-select. Want Hunger + Loneliness (etc.) on the same meal. Follow-up to #764. Empty / «Не указано» stays unset. Existing single-reason meals and backup must keep working. |
