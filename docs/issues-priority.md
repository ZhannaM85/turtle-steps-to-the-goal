...
| # | Status | Issue | Notes |
|---|--------|-------|-------|
| [#939](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/939) | 🔍 Pending validation | PDF diary: excessive blank band below the footer | Fill-strut (`ea6f3f1`) is live and still insufficient on iPhone. Temporary debug overlay + `console.log` of `.pdf-page` layout/capture numbers (enable via Settings PDF debug toggle #952, `?pdfDebug=1`, or `localStorage.pdfDebug=1`); no further capture-height CSS change. Awaiting the on-device dump. |
