...
| [#632](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/632) | 🔲 Open | PDF section picker: some toggles enabled without any real entries | Follow-up to #630. Reported live with a screenshot: Body measurements and Alcohol (and by the same look, Night eating) show enabled even though the user hasn't added entries for them — log-only, not yet investigated |
| [#633](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/633) | 🔲 Open | PDF section picker: enabled state doesn't match Settings' What to track | Related to #632. Reported live with two screenshots: Alcohol tracking is off in Settings' "What to track", but the PDF picker's "Алкоголь" toggle still shows enabled — log-only, not yet investigated |
