...
| [#649](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/649) | 🔍 Pending validation | PWA: "update available" banner keeps reappearing even after reloading | Real root cause found after reopening (the #211-explanation was insufficient — confirmed live the user's client stayed on v604 through 3 newer deploys and several explicit reloads over 45+ minutes; confirmed via `WebFetch` that the live `version.json` correctly reports the latest commit, so the server side was never the problem). `reloadForUpdate()` previously trusted the existing service worker to gracefully self-update (`registration.update()` + bounded wait for `controllerchange`), which depends on the CDN serving fresh `sw.js` bytes on that specific request — rewritten to unconditionally unregister every SW registration and clear every cache before reloading instead, since `useAppUpdateAvailable`'s separate `version.json` check has already confirmed an update exists by the time this ever runs, so there's nothing left to be gentle about |
