...
| # | Status | Issue | Notes |
|---|--------|-------|-------|
| [#631](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/631) | 🔍 Pending validation | Unusual-data-point links can land on a day with no logged data | Reopened once (label/link mismatch — see archive). `CorrelationView.tsx`'s `formatLabel`/`formatNotePreview` now read off the same resolved-date helper as `getDate`, instead of `point.weekStart` — chip label, note preview, and link all agree on the day within the flagged week that actually has the logged weight. Awaiting on-device confirmation |
...
| [#636](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/636) | 📋 Not started | Recipe ingredient copy: show a visible 'Copied' tooltip/toast (#611 follow-up) | Reported live 2026-08-06 during #611 validation — copy confirmation today is only the button's own icon silently swapping clipboard→checkmark for 2s, easy to miss. Needs real on-screen text confirmation, same shape as `MealList.tsx`'s existing toast |
...
| [#638](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/638) | 📋 Not started | Settings 'What to track' night-eating toggle ignores Profile gender | Reported live 2026-08-06 with screenshot — Profile set to Мужской still shows "Ел(а)" (neutral fallback) on the Settings toggle. Root cause confirmed: `SettingsScreen.tsx:691` calls `nightEatingLabel()` with no `sex` argument and never reads `useProfileStore`, unlike every other caller (`DayDetail.tsx`, `DailyEntryFormBottom.tsx`, correlation/chart views) which all pass it through correctly. `exportPdf.ts`/`PdfSectionsDialog.tsx` have the same gap, worth checking in the same pass |
