...
| # | Status | Issue | Notes |
|---|--------|-------|-------|
| [#864](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/864) | 🔍 Pending validation | Enforce max 500 lines per source file + split offenders | ESLint `max-lines` 500 (physical) on `**/*.{ts,tsx}`; temporary allowlist for current offenders only (chip away, do not grow). Dictionaries / `releaseNotes.ts` stay allowlisted until next touched. #868 tracks remaining Day/export/settings gods. |
