...
| # | Status | Issue | Notes |
|---|--------|-------|-------|
| [#771](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/771) | 🔍 Pending validation | AutoSleep Today screenshot fill: deep sleep 7h 10m instead of 45 minutes (z icon) | On-device 2026-08-29: total sleep 9h 22m correct; confirm dialog deep sleep 7h 10m (star / quality) instead of 0h 45m (circled z). Follow-up to #762. Sister: #772. A `Xh Ym` on a hypnogram `DEEP` line is ignored when it is ≥ 4h (quality-sized); deep is the shortest remaining duration (0h 45m). |
