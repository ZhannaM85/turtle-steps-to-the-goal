...
| [#684](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/684) | 🔍 Pending validation | Food library: sort by title (A–Z / Z–A) and by date added | Implemented in Settings → Dishes (`MealItemsSection`): native Sort select near count/search — title A→Z / Z→A and date added newest↔oldest. Preference persisted via `useMealLibrarySortStore` (`turtle-steps-meal-library-sort`). Pure helper `sortMealLibraryItems`. Awaiting on-device confirmation |
