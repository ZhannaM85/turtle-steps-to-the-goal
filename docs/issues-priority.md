...
| [#964](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/964) | 🔍 Pending validation | Settings Export: break down device storage use (and check if PDFs inflate it) | Settings now charts estimated IndexedDB records, offline cache, and other browser storage beside the usage total. PDFs are generated temporarily and saved outside site storage, so the app stores 0 B of PDF exports. Awaiting on-device confirmation. |
