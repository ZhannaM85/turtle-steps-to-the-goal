...
| [#102](https://github.com/ZhannaM85/turtle-steps-to-the-goal/issues/102) | 🔲 Open | Blank screen when opening the app from the iOS home-screen shortcut | Only via the home-screen shortcut, not a regular Safari tab; reload doesn't fix it; happens on cold open. Not yet root-caused — no PWA manifest/`apple-mobile-web-app-capable` tag exists, GitHub Pages overwrites hashed build assets each deploy (stale-cache 404 risk), and there's no React error boundary anywhere so any startup crash is currently silent |
