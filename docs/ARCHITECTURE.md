...
#### `db.ts`
**Why it exists:** Single definition of the IndexedDB schema, now on its fifth version.
...
Vitest + jsdom + `fake-indexeddb` + React Testing Library + `@testing-library/user-event`. **413 tests across 59 files**, all passing as of issue #81. `FoodPickerDialog.test.tsx` sets a 15s file-level `testTimeout` (`vi.setConfig`) — every test there renders the 300+ item list open, which is fast in isolation but was flaky under full-suite parallel load at the default 5s.
