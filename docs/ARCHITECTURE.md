...
| `AppShell.tsx` | One shared `useNavItems(t)` array (Today/Dashboard/History/Goal/Settings/About — no Export entry) drives both a header `<nav aria-label="Main">` (hidden below `sm:`, **sticky** — `sticky top-0 z-10 border-b border-border bg-background`, #25) and a fixed bottom `<nav aria-label="Tabs">` tab bar (visible only below `sm:`, `lucide-react` icon + label per route). **#112**: tabs are `min-h-[106px]` (up from `min-h-14`/56px — bigger tap targets), and the nav accounts for all three horizontal/vertical safe-area insets (`env(safe-area-inset-left/right/bottom)`, previously only bottom), plus a `px-2` on the tab `<ul>` so the edge tabs aren't flush against the viewport edge even without a safe-area inset. `<main>` gets `pb-44 sm:pb-10` (up from `pb-28`) so content still clears the now-taller fixed bar on mobile. **#115**: renders `<AppUpdateBanner />` above the header (see below). |
...
| PWA/home-screen assets (#103) | `index.html` links `apple-touch-icon.png` (180×180), `manifest.json`, and sets `apple-mobile-web-app-capable`/`apple-mobile-web-app-title`/`theme-color` — previously missing entirely, so "Add to Home Screen" fell back to iOS's generic gray-tile icon and a raw, unspaced `<title>` as the name. `public/manifest.json` adds `icon-192.png`/`icon-512.png` for broader PWA support beyond iOS. All three PNGs (`apple-touch-icon.png`, `icon-192.png`, `icon-512.png`) are a simple turtle mark (oval shell + head + 4 legs, pond-mood `--primary`/`--background` tokens) generated by a one-off Node script using only the built-in `zlib` module (no image-library dependency added) — a hand-rolled CRC32 + PNG chunk writer, `zlib.deflateSync` for the IDAT compression. `favicon.svg` updated to the same mark (was a bare placeholder `<rect>` with no fill). Root-relative asset paths (`/apple-touch-icon.png` etc.) are correctly rewritten by Vite to include the `--base=/turtle-steps-to-the-goal/` deploy prefix at build time, same as the pre-existing `/favicon.svg` reference. |
