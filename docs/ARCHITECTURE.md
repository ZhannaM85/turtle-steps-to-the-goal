...
| `FoodPickerDialog.tsx` | Quantity-based entry against the static food list (#62): a Radix Dialog (`shared/ui/dialog.tsx`, #55) with a search input filtering `src/data/foods.ts` by the current locale's name, a scrollable result list, and a grams-quantity field that appears once a food is picked. On confirm, scales the food's per-100g kcal/protein/fat/carbs by `quantity / 100` and hands the computed numbers plus the food's localized name (as the meal's `note`) back to `DailyEntryForm` via an `onAdd` callback — `addFoodEntry()` there adds it as a normal flat `CalorieEntry` (same shape as manual entry, no new "meal group" data model, per the design note added when #62 was scoped). Does **not** call `touchMealItem` — the food list is a separate, curated reference source, not the user's own meal-name history (`useMealItemStore`), so picking a food doesn't pollute that autocomplete library. |
