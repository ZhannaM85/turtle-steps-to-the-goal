...
describe('MealFrequencyCorrelationView', () => {
  ...
  describe('outlier detection and exclusion (#224 mechanism)', () => {
    ...
    // 9 pairs: 8 following a clean more-meals-means-more-gain pattern
    // (counts 3-5, small positive deltas), plus a 9th whose delta
    // (-13.4kg) is wildly outside that pattern — a clear Y-axis-only
    // outlier (its own meal count, 3, is unremarkable).
    function entriesWithOneOutlier(): DailyEntry[] {
      return [
        ...
        entry(day(6), { weightKg: 81.9, calorieEntries: meals(5) }),
        entry(day(7), { weightKg: 82.8, calorieEntries: meals(5) }),
        entry(day(8), { weightKg: 83.4, calorieEntries: meals(3) }),
  ...
  describe('whole-card show/hide toggle (#247 mechanism)', () => {
    afterEach(() => {
      useDashboardChartVisibilityStore.setState((state) => ({
        visible: { ...state.visible, mealFrequencyCorrelation: true },
      }))
    })
