...
describe('mergeZeppLifePatches', () => {
  it('creates a brand-new entry for a date with no existing DailyEntry', () => {
    ...
    const result = mergeZeppLifePatches(patches, [])
  ...
  it('overwrites just the patched fields on an existing entry, wins on conflict', () => {
    const existing = makeEntry({
      weightKg: 59, // will be overwritten by the Zepp value
    ...
    const result = mergeZeppLifePatches(patches, [existing])

    expect(result.daysUpdated).toBe(1)
    expect(result.entriesToUpsert[0]).toMatchObject({
      id: 'existing-1',
      date: '2026-01-15',
      weightKg: 61.4, // Zepp value wins
      bodyFatPercent: 22,
