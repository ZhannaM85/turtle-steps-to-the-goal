...
describe('fastingWindowPoints', () => {
  ...
  it('treats a past-midnight meal on the same entry as the latest one (#984)', () => {
    const entries = [
    ...
    const points = fastingWindowPoints(entries)
    expect(points[0].fastingHours).toBeCloseTo(12.233, 2)
  })
})
...
describe('fastingHoursBetween (#287)', () => {
  ...
  describe('overnight tail on the same entry (#387 / #984)', () => {
    it('treats a past-midnight meal on the same day as the latest one', () => {
      const previous = { calorieEntries: mealAt('19:41', '01:22') }
      const current = { calorieEntries: mealAt('13:36') }

      expect(fastingHoursBetween(previous, current)).toBeCloseTo(12.233, 2)
    })

    it('leaves an ordinary evening-to-morning gap untouched', () => {
      const previous = { calorieEntries: mealAt('20:00') }
      const current = { calorieEntries: mealAt('08:00') }
    ...
    it('does not wrap an 08:27 breakfast (#756)', () => {
      const previous = { calorieEntries: mealAt('22:54') }
      const current = { calorieEntries: mealAt('08:27') }

      expect(fastingHoursBetween(previous, current)).toBeCloseTo(9.55, 2)
    })

    it('uses 08:27 as the first meal, not 11:00 (#756)', () => {
      const previous = { calorieEntries: mealAt('22:54') }
