export interface TodayDict {
  ...
  /** #894 — readable single-day PDF from the same share sheet. */
  sendDaySavePdfButton: string
  sendDaySavePdfFailed: string
  ...
  /** #934 — Day CTA that opens a calorie→weight overlay. */
  ...
  /** #948 — aria-label for the Week / Month / Year segmented control. */
  completeDayHorizonLabel: string
  completeDayIntakeLabel: string
  completeDayMaintenanceLabel: string
  completeDayDeficitLabel: string
  completeDayChangeLower: (
    amount: string,
    horizon: 'week' | 'month' | 'year',
  ) => string
  completeDayChangeHigher: (
    amount: string,
    horizon: 'week' | 'month' | 'year',
  ) => string
  completeDayChangeSame: (horizon: 'week' | 'month' | 'year') => string
